...
const getRawLocations = () => {
  ...
  for (const loc of locations) {
    const prefecture = loc[0]
    const city = loc[1]
    const url = `https://geoapi.heartrails.com/api/json?method=getTowns&prefecture=${prefecture}&city=${city}`;
...
for (const url of urls) {
  setTimeout(function(){ getLocations(url) }, 5000);
}